...
TEST_P(MergeSortedArrayTest, MergeSortedArray) {
  ...
  EXPECT_EQ(std::count_if(testData.nums1.begin(), testData.nums1.end(), [](int x) { return x != 0; }), m + n);
}

INSTANTIATE_TEST_SUITE_P(LeetCodeTest, MergeSortedArrayTest,
                         ::testing::Values(TestData{{1, 2, 3, 0, 0, 0}, 3, {2, 5, 6}, 3},
                                           TestData{{4, 5, 6, 0, 0, 0}, 3, {1, 2, 3}, 3}, TestData{{1}, 1, {}, 0},
                                           TestData{{0}, 0, {1}, 1}));
